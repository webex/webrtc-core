...
export default (props) => {
  ...
  if (localVideo.current && props.localCameraTrack)
    if (props.localCameraTrack.getEffect('blur')) {
      localVideo.current.srcObject = new MediaStream([
        props.localCameraTrack.getMediaStreamTrackWithEffects(),
      ]);
    } else {
      localVideo.current.srcObject = new MediaStream([
  ...
  return (
    <Form>
      ...
      <Form.Group>
        <h3>Local Video</h3>
        <video ref={localVideo} id="localVideo" muted={true} autoPlay playsInline></video>
      </Form.Group>
      <Form.Group>
        <h3>Local Screen Share</h3>
        <video ref={localShare} id="localScreenShare" muted={true} autoPlay playsInline></video>
      </Form.Group>
    </Form>
  );
};
